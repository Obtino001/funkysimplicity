    ...
    if (target instanceof HTMLInputElement) {
      ...
      if (!Number.isNaN(fieldsetIndex) && !Number.isNaN(inputIndex)) {
        ...
        if (radios && checkedIndices && fieldset) {
          ...
          const legendValue = fieldset.querySelector('.variant-option__selected-value, .variant-option__swatch-value');
          if (legendValue && target.value) {
            legendValue.textContent = target.value;
          }
